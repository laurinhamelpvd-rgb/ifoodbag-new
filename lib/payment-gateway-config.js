...
function resolveGatewayWithFallback(requested, payments = {}, options = {}) {
    ...
    const priority = getGatewayPriority(requested || activeGateway, activeGateway);
...
function buildPaymentsConfig(raw = {}) {
    ...
    const payments = {
        activeGateway: normalizeActiveGatewayId(raw.activeGateway || process.env.PAYMENTS_ACTIVE_GATEWAY || DEFAULT_ACTIVE_GATEWAY),
        gateways: {
            ghostspay,
            sunize,
            paradise,
    ...
    payments.activeGateway = resolveGatewayWithFallback(payments.activeGateway, payments, { enabledByGateway });
    return payments;
}
